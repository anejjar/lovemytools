...
export default function HtmlMinifierTool() {
  ...
  useEffect(() => { setResult(minifyHtml(debounced)); }, [debounced]);

  return (
    <div className="space-y-4">
      {result.originalSize > 0 && (
        <div className="flex flex-wrap gap-4 rounded-xl border border-[var(--border)] bg-[var(--surface2)] px-4 py-3 text-sm">
          {[["Original", `${result.originalSize} B`], ["Minified", `${result.minifiedSize} B`], ["Saved", `${result.savings} B`], ["Reduction", `${result.savingsPercent}%`]].map(([label, val]) => (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="space-y-2">
          ...
          <CodeEditor value={input} onChange={setInput} placeholder={"<div>\n  <!-- comment -->\n  <p>Hello World</p>\n</div>"} rows={18} />
        </div>
        <div className="space-y-2">
          <div className="flex items-center justify-between">
          ...
          <CodeEditor value={result.output} readOnly rows={18} placeholder="Minified HTML will appear here..." />
        </div>
      </div>
